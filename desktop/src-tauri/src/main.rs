...
fn create_person(person: Person, state: State<DbState>) -> Result<Person, String> {
    ...
    conn.execute(
        "INSERT INTO persons (id, first_name, last_name, maiden_name, birth_date, death_date, photo, gender, generation, position_x, position_y, created_at, updated_at)
...
fn get_person(id: String, state: State<DbState>) -> Result<Option<Person>, String> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, first_name, last_name, maiden_name, birth_date, death_date, photo, gender, generation, position_x, position_y, created_at, updated_at 
...
fn update_person(person: Person, state: State<DbState>) -> Result<Person, String> {
    ...
    conn.execute(
        "UPDATE persons SET first_name = ?2, last_name = ?3, maiden_name = ?4, birth_date = ?5, death_date = ?6, photo = ?7, gender = ?8, generation = ?9, position_x = ?10, position_y = ?11, updated_at = ?12
         WHERE id = ?1",
        rusqlite::params![
            person.id,
            person.first_name,
...
fn create_relationship(relationship: Relationship, state: State<DbState>) -> Result<Relationship, String> {
    ...
    conn.execute(
        "INSERT INTO relationships (id, from_person_id, to_person_id, rel_type, spouse_type, start_date, end_date, color, created_at, updated_at)
...
fn get_relationship(id: String, state: State<DbState>) -> Result<Option<Relationship>, String> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, from_person_id, to_person_id, rel_type, spouse_type,
...
fn update_relationship(relationship: Relationship, state: State<DbState>) -> Result<Relationship, String> {
    ...
    conn.execute(
        "UPDATE relationships SET from_person_id = ?2, to_person_id = ?3, rel_type = ?4, spouse_type = ?5, start_date = ?6, end_date = ?7, color = ?8, updated_at = ?9
         WHERE id = ?1",
        rusqlite::params![
            relationship.id,
            relationship.from_person_id,
